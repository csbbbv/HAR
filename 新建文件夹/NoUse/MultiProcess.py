...
def Takedata(ACCX,ACCY,ACCZ,GYROX,GYROY,GYROZ,loc):
    ACClis,GYROlis=[[]]*3,[[]]*3
    loc.acquire()
    if ACCX.full():
        for i in range(100):
    ...
    if index == 0:
        ...
        print("up")
def main(ACCX,ACCY,ACCZ,GYROX,GYROY,GYROZ):

    loc=mp.Lock()
    process1=mp.Process(target=GetDataFromSensor,args=(ACCX,ACCY,ACCZ,GYROX,GYROY,GYROZ,loc))
...
if __name__ == '__main__':
    ...
    GYROZ = Queue(100)
    while True:
        main(ACCX,ACCY,ACCZ,GYROX,GYROY,GYROZ)
